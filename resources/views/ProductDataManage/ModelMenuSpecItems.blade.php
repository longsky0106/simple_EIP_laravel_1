...
<div id="spec_input_content" class="flex flex-wrap w-full">
	<div id="zh-tw_spec" class="flex-1">
        ...
        @foreach($MenuSpecItems as $key => $MenuSpecItem)
            <div class="flex w-full p-1">
                ...
                <input class="flex-1 input input-sm sm:input-md input-bordered" type="text" id="" name="{{$MenuSpecItem->spec_item_name_form}}" 
                        value="@isset($SK_SPEC_tw[$MenuSpecItem->spec_item_name]){{ $SK_SPEC_tw[$MenuSpecItem->spec_item_name] }}@endisset"> <!-- 規格項目數值 -->
	...
	<div id="en-us_spec" class="flex-1">
        ...
        <div class="flex p-1">
            ...
            <input class="flex-1 w-[calc(100%-6rem)] sm:w-[calc(100%-16rem)] input input-sm sm:input-md input-bordered" type="text" id="" name="name_for_sell_en" 
                    value="{{ $name_for_sell_en }}" maxlength="60">
        </div>	
        @foreach($MenuSpecItems as $key => $MenuSpecItem)
            <div class="flex p-1" >
                <label class="hidden sm:flex items-center w-40" for="spec_item_name_en">{{$MenuSpecItem->spec_item_name_en}}</label>
                <input class="flex-1 input input-sm sm:input-md input-bordered" type="text" id="" name="{{$MenuSpecItem->spec_item_name_form}}_en" 
                        value="@isset($SK_SPEC_en[$MenuSpecItem->spec_item_name_en]){{ $SK_SPEC_en[$MenuSpecItem->spec_item_name_en] }}@endisset">
